...
public class BinTree {
    ...
    if (root == null || root.right == null) {
        return; // Ensure tree has a right child for rotation
    }
    BinTreeNode newRoot = root.right;
    root.right = newRoot.left;
    newRoot.left = root;
    root = newRoot; // New root is now the previous right child
    }

public void rightrotate() {
    if (root == null || root.left == null) {
        return; // Ensure tree has a left child for rotation
    }
    BinTreeNode newRoot = root.left;
    root.left = newRoot.right;
    newRoot.right = root;
    root = newRoot; // New root is now the previous left child
    }
	...
	BinTree tree = new BinTree(); // Tree to test.
        ...
        while (ans != 9) {
	      ...
	      System.out.println("The value is not in the tree.");
            ...
            tree.mirrorImage();
            System.out.println("Free nodes = " + tree.countFree());
          }
          ans = menu();        
        }        
